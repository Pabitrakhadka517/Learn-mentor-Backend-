...
class AuthRepository {
    ...
    static async findRefreshTokenByCompare(userId, rawToken) {
        const storedTokens = await user_model_1.RefreshToken.find({
            userId,
            expiresAt: { $gt: new Date() },
        });
        for (const stored of storedTokens) {
            const isMatch = await bcryptjs_1.default.compare(rawToken, stored.tokenHash);
            if (isMatch)
                return stored;
        }
        return null;
    ...
    static async findPasswordResetToken(plainToken) {
        const currentTime = new Date();
        const tokenHash = require('crypto').createHash('sha256').update(plainToken).digest('hex');
        const resetToken = await user_model_1.PasswordResetToken.findOne({
            tokenHash: tokenHash,
            used: false,
